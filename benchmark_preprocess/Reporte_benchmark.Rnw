...
Para el análisis de variantes en número de copias (CNVs) se utilizó el \textit{plugin cnvPartition CNV Analysis} versión 3.2.1 y se agregaron a la tabla de resultados las columnas de \textit{CNV Value} y \textit{CNV Confidence}, la cual se exportó en formato TXT (\texttt{\Sexpr{i=+1; name_tablas[i]}}).
...
Los CSV con la proporciones de diferencias por muestra, frecuencia absoluta de diferencia y número de variantes por frecuencia de diferencia se encuentran en los archivos \texttt{\Sexpr{i=+1; name_tablas[i]}}, \texttt{\Sexpr{i=+1; name_tablas[i]}} y \texttt{\Sexpr{i=+1; name_tablas[i]}}, respectivamente.
